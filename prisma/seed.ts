...
async function main() {
  ...
  const cashier = await  addTestCashier(branch.id);
...
const addTestCashier = async (branchId: string) : Promise<User>  => {
  const password = await bcrypt.hash('12345678', 10);

  const user = await db.user.create({
    data: {
      fullName: "Bethlhem Tesfa",
      email: "cashier@apollo.com",
  ...
  await db.cashierAccount.create({
    data: {
      branchId,
      userId: user.id
    },
